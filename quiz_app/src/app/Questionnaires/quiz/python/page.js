...
import pythonQuiz from '../../question_crypto/python';
...
export default function PythonQuiz() {
    ...
    const handleAnswerClick = (selectedOption) => {
        ...
        if (selectedOption === pythonQuiz[currentQuestion].correctAnswer) {
            setScore(prevScore => prevScore + 100);
        }

        setTimeout(() => {
            const nextQuestion = currentQuestion + 1;
            if (nextQuestion < pythonQuiz.length) {
    ...
    const getButtonColor = (option) => {
        ...
        if (option.charAt(0) === pythonQuiz[currentQuestion].correctAnswer) {
    ...
    if (!pythonQuiz || pythonQuiz.length === 0) {
        return (
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    <p className="text-xl text-center text-red-500 font-bold">Bonnes réponses : <span className="font-bold text-violet-600">{score / 100} sur {pythonQuiz.length}</span></p>
    ...
    const currentQuestionData = pythonQuiz[currentQuestion];
                ...
                <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="text-2xl font-bold text-violet-800">
                            Question {currentQuestion + 1} / {pythonQuiz.length}
                        </div>
                        <motion.div 
